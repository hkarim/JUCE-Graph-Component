...
struct MidiInNodeProcessor : public NodeProcessor {

  explicit MidiInNodeProcessor(Graph *graph) :
    NodeProcessor(graph) {
  }

  MidiInNodeProcessor(Graph *graph, const std::string &name, uint32_t n_ins, uint32_t n_outs)
    : NodeProcessor(graph, name, n_ins, n_outs) {
  }

  ~MidiInNodeProcessor() override = default;
  ...
  [[nodiscard]] std::string typeId() const override {
    return Processors::midiInNodeProcessor;
  }
