...
namespace ce
{
  ...
  public:
    ...
    void drawReadableSingleLineText(juce::Graphics& g, const typename CurveEditorModel<T>::PointType& baseline, const std::string& text,
  ...
  template <typename T>
  void CurveEditor<T>::drawReadableSingleLineText(juce::Graphics& g, const typename CurveEditorModel<T>::PointType& baseline,
    ...
    if (selectedHandle) {
      ...
      if (selectedHandle == &selectedHandle->parent->anchor) {
        ...
        if (prevNode) {
          ...
          selectedHandle->parent->control1.pt.setX(juce::jmax(selectedHandle->parent->control1.pt.getX(), prevNode->anchor.pt.getX()));
          selectedHandle->parent->control2.pt.setX(juce::jmax(selectedHandle->parent->control2.pt.getX(), prevNode->anchor.pt.getX()));
        ...
        if (nextNode) {
          ...
          selectedHandle->parent->control1.pt.setX(juce::jmin(selectedHandle->parent->control1.pt.getX(), nextNode->anchor.pt.getX()));
          selectedHandle->parent->control2.pt.setX(juce::jmin(selectedHandle->parent->control2.pt.getX(), nextNode->anchor.pt.getX()));
  ...
  template <typename T>
  void CurveEditor<T>::mouseUp(const juce::MouseEvent& event) {
    selectedHandle = nullptr;
    repaint();
  }
  ...
  template <typename T>
  void CurveEditor<T>::mouseMove(const juce::MouseEvent& event) {
    repaint();
  }
  ...
  template <typename T>
  void CurveEditor<T>::valueChanged(juce::Value& value) {
    repaint();
  }
